...
const empty = { code: '', name_en: '', name_ar: '', phone: '', email: '', address: '', tax_no: '', payment_terms_days: 30, opening_balance: 0 };
...
export default function SuppliersPage() {
  ...
  const load = () => { setLoading(true); api.get('/suppliers').then((r) => setItems(r.data)).finally(() => setLoading(false)); };
  ...
  const remove = async () => { await api.delete(`/suppliers/${toDelete.id}`); toast.success('Deactivated'); setToDelete(null); load(); };
  ...
  return (
    <div>
      <PageHeader title={t('nav.suppliers')} actions={canCreateEdit && <button onClick={openNew} className="btn-primary"><Plus size={16} /> {t('common.add')}</button>} />
      ...
      <SlideOver open={open} onClose={() => setOpen(false)} title={editing ? t('common.edit') : t('common.add')} onSubmit={submit} submitting={saving}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="label">Payment Terms (days)</label><input type="number" className="input" value={form.payment_terms_days} onChange={(e) => setForm({ ...form, payment_terms_days: e.target.value })} /></div>
