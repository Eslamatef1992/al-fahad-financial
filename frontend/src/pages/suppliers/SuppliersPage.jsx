...
const empty = { code: '', name_en: '', name_ar: '', phone: '', email: '', address: '', tax_no: '', payment_terms_days: 30, opening_balance: 0, parent_account_id: null };
...
export default function SuppliersPage() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/suppliers', { params: showInactive ? { status: 'all' } : {} }).then((r) => setItems(r.data)).finally(() => setLoading(false));
  };
  useEffect(() => { if (activeCompany) load(); }, [activeCompany, showInactive]);

  const openNew = () => { setEditing(null); setForm({ ...empty, parent_account_id: financialDefaults.supplier_parent_account_id || null }); setOpen(true); };
  ...
  const toggleActive = async (row) => {
    await api.put(`/suppliers/${row.id}`, { is_active: !row.is_active });
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <button onClick={() => printFile('/suppliers/pdf', {})} className="btn-ghost"><Printer size={16} /> {t('common.print')}</button>
            <button onClick={() => downloadFile('/suppliers/excel', {}, 'suppliers.xlsx')} className="btn-ghost"><Download size={16} /> {t('common.excel')}</button>
      ...
      <SlideOver open={open} onClose={() => setOpen(false)} title={editing ? t('common.edit') : t('common.add')} onSubmit={submit} submitting={saving}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="label">{t('common.paymentTermsDays')}</label><input type="number" className="input" value={form.payment_terms_days} onChange={(e) => setForm({ ...form, payment_terms_days: e.target.value })} /></div>
