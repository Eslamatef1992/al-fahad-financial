...
const TYPE_COLORS = {
  ...
  equity: 'bg-purple-50 text-purple-600', revenue: 'bg-emerald-50 text-emerald-600',
...
export default function TreeNode({ node, depth = 0, onAddChild, onEdit, onDelete }) {
  ...
  return (
    <div>
      <div
        className="group flex items-center gap-2 py-2 px-2 rounded-lg hover:bg-slate-50 dark:hover:bg-navy-800/50 transition-colors"
      ...
      >
        ...
        {node.is_group ? <FolderTree size={15} className="text-navy-500 shrink-0" /> : <FileText size={14} className="text-slate-400 shrink-0" />}
