...
export default function VehiclesPage() {
  ...
  const openNew = () => { setEditing(null); setForm({
    ...empty,
    parent_account_id: financialDefaults.vehicle_parent_account_id || null,
    secondary_parent_account_id: financialDefaults.vehicle_secondary_parent_account_id || null,
    tertiary_parent_account_id: financialDefaults.vehicle_tertiary_parent_account_id || null,
  }); setOpen(true); };
  const openEdit = (row) => {
    setEditing(row);
    setForm({
      ...empty, ...row,
      parent_account_id: row.account?.parent_id || null,
      secondary_parent_account_id: row.secondaryAccount?.parent_id || null,
      tertiary_parent_account_id: row.tertiaryAccount?.parent_id || null,
    });
    setOpen(true);
  };
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <button onClick={() => printFile('/vehicles/pdf', {})} className="btn-ghost"><Printer size={16} /> {t('common.print')}</button>
            <button onClick={() => downloadFile('/vehicles/excel', {}, 'vehicles.xlsx')} className="btn-ghost"><Download size={16} /> {t('common.excel')}</button>
