...
export default function VoucherDetailPage() {
  ...
  const act = async () => {
    ...
    toast.success(confirmAction === 'post' ? 'Voucher posted to ledger' : 'Voucher cancelled');
    setConfirmAction(null);
    load();
  };

  if (!voucher) return <p className="text-slate-400">Loading...</p>;
  ...
  return (
    <div>
      ...
      <div className="card p-5 mb-5 flex flex-wrap items-center gap-6">
        <div><p className="text-xs text-slate-400 uppercase font-semibold">Status</p><span className={`inline-block mt-1 px-3 py-1 rounded-full text-xs font-bold capitalize ${STATUS_COLOR[voucher.status]}`}>{voucher.status}</span></div>
        <div><p className="text-xs text-slate-400 uppercase font-semibold">Description</p><p className="text-sm mt-1">{voucher.description || '—'}</p></div>
        <div><p className="text-xs text-slate-400 uppercase font-semibold">Total</p><p className="text-sm mt-1 font-bold">{Number(voucher.total_debit).toFixed(3)}</p></div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 dark:border-navy-800">
              ...
              <th className="px-4 py-3 text-start text-xs font-semibold text-slate-500 uppercase">Cost Center</th>
      ...
      <ConfirmDialog
        ...
        onConfirm={act}
        message={confirmAction === 'post' ? 'Post this voucher to the general ledger? This cannot be edited afterward.' : 'Cancel this posted voucher? A reversing entry will be created.'}
      />
    </div>
  );
}
