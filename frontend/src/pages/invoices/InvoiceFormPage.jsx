...
export default function InvoiceFormPage() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!header[partyKey]) return toast.error(`Please select a ${type === 'sales' ? 'client' : 'supplier'}`);
  ...
  return (
    <div>
      <button onClick={() => navigate(`/invoices/${type}`)} className="btn-ghost !px-2 mb-3"><ArrowLeft size={16} /> Back</button>
      <PageHeader title={type === 'sales' ? 'New Sales Invoice' : 'New Purchase Bill'} />

      <form onSubmit={submit} className="space-y-5">
        <div className="card p-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="label">{type === 'sales' ? 'Client' : 'Supplier'}</label>
          ...
          <div><label className="label">Reference No.</label><input className="input" value={header.reference_no} onChange={(e) => setHeader({ ...header, reference_no: e.target.value })} /></div>
          ...
          <div><label className="label">Due Date</label><input type="date" className="input" value={header.due_date} onChange={(e) => setHeader({ ...header, due_date: e.target.value })} /></div>
          <div>
          ...
          <div className="sm:col-span-2"><label className="label">Notes</label><textarea className="input" rows={2} value={header.notes} onChange={(e) => setHeader({ ...header, notes: e.target.value })} /></div>
        </div>
        ...
        <div className="card p-5">
            ...
            {lines.map((line, idx) => (
              <motion.div key={idx} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-12 gap-2 items-start p-3 rounded-xl bg-slate-50 dark:bg-navy-800/40">
                <select required className="input col-span-3 !py-2" value={line.account_id} onChange={(e) => updateLine(idx, { account_id: e.target.value })}>
                  <option value="">{type === 'sales' ? 'Revenue account...' : 'Expense account...'}</option>
                  {accounts.map((a) => <option key={a.id} value={a.id}>{a.code} - {a.name_en}</option>)}
                </select>
                <input placeholder="Description" className="input col-span-3 !py-2" value={line.description} onChange={(e) => updateLine(idx, { description: e.target.value })} />
                <input type="number" step="0.001" placeholder="Qty" className="input col-span-1 !py-2" value={line.quantity} onChange={(e) => updateLine(idx, { quantity: e.target.value })} />
                <input type="number" step="0.001" placeholder="Unit Price" className="input col-span-2 !py-2" value={line.unit_price} onChange={(e) => updateLine(idx, { unit_price: e.target.value })} />
                <input type="number" step="0.01" placeholder="Tax %" className="input col-span-1 !py-2" value={line.tax_rate} onChange={(e) => updateLine(idx, { tax_rate: e.target.value })} />
          ...
          <div className="flex items-center justify-end gap-6 mt-4 pt-4 border-t border-slate-100 dark:border-navy-800 text-sm">
            <div><span className="text-slate-400">Subtotal: </span><span className="font-semibold">{subtotal.toFixed(3)}</span></div>
            <div><span className="text-slate-400">Tax: </span><span className="font-semibold">{taxTotal.toFixed(3)}</span></div>
            <div><span className="text-slate-400">Total: </span><span className="font-bold text-base">{total.toFixed(3)}</span></div>
          </div>
        </div>

        <div className="flex justify-end">
          <button disabled={saving} className="btn-primary"><Save size={16} /> Save as Draft</button>
        </div>
      </form>
    </div>
  );
